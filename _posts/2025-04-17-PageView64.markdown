...
u8* vtop(EPROCESS* proc, virt_addr_t va)
{
  ...
  pml4e_t* pml4 = read(_pfn_to_page(proc->DirectoryTableBase.pfn));
  pml3e_t* pml3 = read(_pfn_to_page(pml4[va.pml4_idx].pfn));
  pml2e_t* pml2 = read(_pfn_to_page(pml3[va.pml3_idx].pfn));
  pml1e_t* pml1 = read(_pfn_to_page(pml2[va.pml2_idx].pfn));

  // we also assume all pages are paged-in (present=1)...
  return (u8*)(_pfn_to_page(pml1[va.pml1_idx].pfn) + va.offset);
}
{% endhighlight %}
